...
const std::string &InputFile0 =
    Path + std::string("CircuitBoard_2048x1024_8u.raw");

const std::string &LabelMarkersOutputFile0 =
    Path + std::string("CircuitBoard_LabelMarkersUF_8Way_2048x1024_32u.raw");

const std::string &CompressedMarkerLabelsOutputFile0 =
    Path +
    std::string("CircuitBoard_CompressedMarkerLabelsUF_8Way_2048x1024_32u.raw");

const std::string &ContoursOutputFile0 =
    Path + std::string("CircuitBoard_Contours_8Way_2048x1024_8u.raw");

const std::string &ContoursDirectionOutputFile0 =
    Path + std::string("CircuitBoard_ContoursDirection_8Way_2048x1024_8u.raw");

const std::string &ContoursReconstructedFile0 =
    Path +
    std::string("CircuitBoard_ContoursReconstructed_8Way_2048x1024_8u.raw");
...
int main(int argc, char **argv) {
  ...
  cudaError = cudaMalloc((void **)&pInputImageDev,
                         oSizeROI.width * sizeof(Npp8u) * oSizeROI.height);
  if (cudaError != cudaSuccess)
  ...
  // Also the image pointer used for label markers generation output must start
  // at the same position in the image as it does in the marker compression
  // function.  Also note that actual input image size and ROI do not
  // necessarily need to be related other than ROI being less than or equal to
  // image size and image starting position does not necessarily have to be at
  // pixel 0 in the input image.

  cudaError = cudaMalloc((void **)&pUFLabelDev,
                         oSizeROI.width * sizeof(Npp32u) * oSizeROI.height);
  if (cudaError != cudaSuccess)
    return NPP_MEMORY_ALLOCATION_ERR;

  pInputImageHost = reinterpret_cast<Npp8u *>(
      malloc(oSizeROI.width * sizeof(Npp8u) * oSizeROI.height));
  pUFLabelHost = reinterpret_cast<Npp32u *>(
      malloc(oSizeROI.width * sizeof(Npp32u) * oSizeROI.height));

  // Use UF functions throughout this sample.

  nppStatus = nppiLabelMarkersUFGetBufferSize_32u_C1R(
      oSizeROI, &aGenerateLabelsScratchBufferSize);

  // One at a time image processing

  cudaError = cudaMalloc((void **)&pUFGenerateLabelsScratchBufferDev,
                         aGenerateLabelsScratchBufferSize);
  if (cudaError != cudaSuccess)
    return NPP_MEMORY_ALLOCATION_ERR;

  if (loadRaw8BitImage(pInputImageHost, oSizeROI.width * sizeof(Npp8u),
                       oSizeROI.height, 0) == 0) {
    cudaError = cudaMemcpy2DAsync(
        pInputImageDev, oSizeROI.width * sizeof(Npp8u), pInputImageHost,
        oSizeROI.width * sizeof(Npp8u), oSizeROI.width * sizeof(Npp8u),
        oSizeROI.height, cudaMemcpyHostToDevice, nppStreamCtx.hStream);

    nppStatus = nppiLabelMarkersUF_8u32u_C1R_Ctx(
        pInputImageDev, oSizeROI.width * sizeof(Npp8u), pUFLabelDev,
        oSizeROI.width * sizeof(Npp32u), oSizeROI, nppiNormInf,
        pUFGenerateLabelsScratchBufferDev, nppStreamCtx);
    ...
    cudaError = cudaMemcpy2DAsync(
        pUFLabelHost, oSizeROI.width * sizeof(Npp32u), pUFLabelDev,
        oSizeROI.width * sizeof(Npp32u), oSizeROI.width * sizeof(Npp32u),
        oSizeROI.height, cudaMemcpyDeviceToHost, nppStreamCtx.hStream);
    ...
    for (int j = 0; j < oSizeROI.height; j++) {
      nSize += fwrite(&pUFLabelHost[j * oSizeROI.width], sizeof(Npp32u),
                      oSizeROI.width, bmpFile);
    }
    fclose(bmpFile);

    nppStatus = nppiCompressMarkerLabelsGetBufferSize_32u_C1R(
        oSizeROI.width * oSizeROI.height, &aCompressLabelsScratchBufferSize);
    if (nppStatus != NPP_NO_ERROR)
      return nppStatus;

    cudaError = cudaMalloc((void **)&pUFCompressedLabelsScratchBufferDev,
    ...
    nppStatus = nppiCompressMarkerLabelsUF_32u_C1IR_Ctx(
        pUFLabelDev, oSizeROI.width * sizeof(Npp32u), oSizeROI,
        oSizeROI.width * oSizeROI.height, &nCompressedLabelCount,
        pUFCompressedLabelsScratchBufferDev, nppStreamCtx);

    if (nppStatus != NPP_SUCCESS) {
      printf(
          "CircuitBoard_CompressedLabelMarkersUF_8Way_2048x1024_32u failed.\n");
      tearDown();
      return -1;
    }
    ...
    printf("CircuitBoard_CompressedMarkerLabelsUF_8Way_2048x1024_32u "
           "succeeded, compressed label count is %d.\n",
           nCompressedLabelCount);

    unsigned int nInfoListSize;

    nppStatus = nppiCompressedMarkerLabelsUFGetInfoListSize_32u_C1R(
        nCompressedLabelCount, &nInfoListSize);
    if (nppStatus != NPP_NO_ERROR)
      return nppStatus;
    ...
    cudaError = cudaMalloc((void **)&pContoursImageDev,
                           oSizeROI.width * sizeof(Npp8u) * oSizeROI.height);
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    cudaError =
        cudaMalloc((void **)&pContoursDirectionImageDev,
                   oSizeROI.width * sizeof(NppiContourPixelDirectionInfo) *
                       oSizeROI.height);
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    cudaError = cudaMalloc((void **)&pContoursPixelCountsListDev,
                           sizeof(Npp32u) * (nCompressedLabelCount + 4));
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    cudaError = cudaMalloc((void **)&pContoursPixelsFoundListDev,
                           sizeof(Npp32u) * (nCompressedLabelCount + 4));
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    cudaError = cudaMalloc((void **)&pContoursPixelStartingOffsetDev,
                           sizeof(Npp32u) * (nCompressedLabelCount + 4));
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    pMarkerLabelsInfoListHost =
        reinterpret_cast<NppiCompressedMarkerLabelsInfo *>(
            malloc(nInfoListSize));
    pContoursImageHost = reinterpret_cast<Npp8u *>(
        malloc(oSizeROI.width * sizeof(Npp8u) * oSizeROI.height));
    pContoursDirectionImageHost =
        reinterpret_cast<NppiContourPixelDirectionInfo *>(
            malloc(oSizeROI.width * sizeof(NppiContourPixelDirectionInfo) *
                   oSizeROI.height));
    pContoursPixelCountsListHost = reinterpret_cast<Npp32u *>(
        malloc(sizeof(Npp32u) * (nCompressedLabelCount + 4)));
    pContoursPixelsFoundListHost = reinterpret_cast<Npp32u *>(
        malloc(sizeof(Npp32u) * (nCompressedLabelCount + 4)));
    pContoursPixelStartingOffsetHost = reinterpret_cast<Npp32u *>(
        malloc(sizeof(Npp32u) * (nCompressedLabelCount + 4)));

#ifdef USE_NPP_11_5
    nppStatus = nppiCompressedMarkerLabelsUFInfo_32u_C1R_Ctx(
        pUFLabelDev, oSizeROI.width * sizeof(Npp32u), oSizeROI,
        nCompressedLabelCount, pMarkerLabelsInfoListDev, pContoursImageDev,
        oSizeROI.width * sizeof(Npp8u), pContoursDirectionImageDev,
        oSizeROI.width * sizeof(NppiContourPixelDirectionInfo),
        &oContoursTotalsInfoHost, pContoursPixelCountsListDev,
        pContoursPixelCountsListHost, pContoursPixelStartingOffsetDev,
    ...
    nppStatus = nppiCompressedMarkerLabelsUFInfo_32u_C1R_Ctx(
        ...
        oSizeROI.width * sizeof(NppiContourPixelDirectionInfo),
        &oContoursTotalsInfoHost, pContoursPixelCountsListDev,
        pContoursPixelCountsListHost, NULL, pContoursPixelStartingOffsetHost,
        nppStreamCtx);
#endif
    cudaError = cudaMemcpy2DAsync(
        pContoursImageHost, oSizeROI.width * sizeof(Npp8u), pContoursImageDev,
        oSizeROI.width * sizeof(Npp8u), oSizeROI.width * sizeof(Npp8u),
        oSizeROI.height, cudaMemcpyDeviceToHost, nppStreamCtx.hStream);
    ...
    cudaError = cudaMemcpy2DAsync(
        ...
        oSizeROI.width * sizeof(NppiContourPixelDirectionInfo),
        oSizeROI.width * sizeof(NppiContourPixelDirectionInfo), oSizeROI.height,
    ...
    nSize = 0;
    for (int j = 0; j < oSizeROI.height; j++) {
      nSize += fwrite(&pContoursImageHost[j * oSizeROI.width], sizeof(Npp8u),
                      oSizeROI.width, bmpFile);
    }
    fclose(bmpFile);
    ...
    nppStatus = nppiCompressedMarkerLabelsUFGetContoursBlockSegmentListSize_C1R(
        pContoursPixelCountsListHost,
        oContoursTotalsInfoHost.nTotalImagePixelContourCount,
        nCompressedLabelCount, nStartID, nStopID,
        &nContoursBlockSegmentListSize);

    cudaError = cudaMalloc((void **)&pContoursBlockSegmentListDev,
                           nContoursBlockSegmentListSize);
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    pContoursBlockSegmentListHost = reinterpret_cast<NppiContourBlockSegment *>(
        malloc(nContoursBlockSegmentListSize));
#endif

    unsigned int nGeometryListSize;

    nppStatus = nppiCompressedMarkerLabelsUFGetGeometryListsSize_C1R(
        pContoursPixelStartingOffsetHost[nCompressedLabelCount],
        &nGeometryListSize);

    NppiContourPixelGeometryInfo *pContoursPixelGeometryListsHost =
        reinterpret_cast<NppiContourPixelGeometryInfo *>(
            malloc(nGeometryListSize));
    NppiContourPixelGeometryInfo *pContoursPixelGeometryListsDev;

    cudaError =
        cudaMalloc((void **)&pContoursPixelGeometryListsDev, nGeometryListSize);
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;
    ...
    cudaError =
        cudaMemcpyAsync(pContoursDirectionImageHost, pContoursDirectionImageDev,
                        oSizeROI.width * sizeof(NppiContourPixelDirectionInfo) *
                            oSizeROI.height,
                        cudaMemcpyDeviceToHost, nppStreamCtx.hStream);

    pContoursGeometryImageHost = reinterpret_cast<Npp8u *>(
        malloc(oSizeROI.width * sizeof(Npp8u) * oSizeROI.height));
    pContoursOrderedGeometryImageHost = reinterpret_cast<Npp8u *>(
        malloc(oSizeROI.width * sizeof(Npp8u) * oSizeROI.height));

    if (pContoursOrderedGeometryImageHost != 0)
      memset(pContoursOrderedGeometryImageHost, 255,
             oSizeROI.width * sizeof(Npp8u) * oSizeROI.height);

    cudaStreamSynchronize(nppStreamCtx.hStream);

/*
 * Note that to significantly improve performance by default a contour that
 * contains more than 256K pixels will be bypassed when generating the output
 * geometry list. The contour ID and number of contour pixels will be output in
 * the contour list however. You can still get this function to output the
 * geometry of this size contour however by calling the function with a starting
 * contour ID of that contour ID and ending contour ID of that contour ID + 1.
 * Note that doing so for contours approaching a million pixels can take many
 * minutes. Also, due to the structure of some images contour ID 0 can contain
 * ALL contours in the image so setting the starting contour ID to 1 can
 * significantly increase output preprocessing performance.
 *
 * Once nppiCompressedMarkerLabelsUFContoursGenerateGeometryLists_C1R_Ctx() has
 * been called with a particular range of contour IDs
 * nppiCompressedMarkerLabelsUFContoursOutputGeometryLists_C1R() can be recalled
 * any number of times with any range of contour IDs that were included in the
 * preceeding corresponding
 * nppiCompressedMarkerLabelsUFContoursGenerateGeometryLists_C1R_Ctx() call.
 *
 * In general a few contour pixels can escape insertion into the ordered
 * geometry list.  When geometry output is in clockwise order the extra pixels
 * will be at the end of the geometry list otherwise they will be at the start
 * of the list.
 *
 */
#ifdef USE_NPP_11_5
    nppStatus =
        nppiCompressedMarkerLabelsUFContoursGenerateGeometryLists_C1R_Ctx(
            pMarkerLabelsInfoListDev, pMarkerLabelsInfoListHost,
            pContoursDirectionImageDev,
            oSizeROI.width * sizeof(NppiContourPixelDirectionInfo),
            pContoursPixelGeometryListsDev, pContoursPixelGeometryListsHost,
            pContoursGeometryImageHost, oSizeROI.width * sizeof(Npp8u),
            pContoursPixelCountsListDev, pContoursPixelsFoundListDev,
            pContoursPixelsFoundListHost, pContoursPixelStartingOffsetDev,
            pContoursPixelStartingOffsetHost,
            oContoursTotalsInfoHost.nTotalImagePixelContourCount,
            nCompressedLabelCount, nStartID, nStopID,
            pContoursBlockSegmentListDev, pContoursBlockSegmentListHost,
    ...
    nppStatus =
        nppiCompressedMarkerLabelsUFContoursGenerateGeometryLists_C1R_Ctx(
            ...
            pContoursDirectionImageHost,
            oSizeROI.width * sizeof(NppiContourPixelDirectionInfo),
            pContoursPixelGeometryListsHost, pContoursPixelCountsListHost,
            pContoursPixelsFoundListHost, pContoursPixelStartingOffsetHost,
            nCompressedLabelCount, nStartID, nStopID, oSizeROI, nppStreamCtx);
    ...
    nppStatus = nppiCompressedMarkerLabelsUFContoursOutputGeometryLists_C1R(
        pContoursPixelGeometryListsHost, pMarkerLabelsInfoListHost,
        pContoursGeometryImageHost, oSizeROI.width * sizeof(Npp8u),
        pContoursPixelCountsListHost, pContoursPixelsFoundListHost,
        pContoursPixelStartingOffsetHost, nCompressedLabelCount, nStartID,
        nStopID,
        1, // Counterclockwise contoure geometry list output
        oSizeROI);
    ...
    for (unsigned int nID = nStartID; nID < nStopID; nID++) {
      NppiContourPixelGeometryInfo *pCurContoursPixelGeometryListHost =
          &pContoursPixelGeometryListsHost
              [pContoursPixelStartingOffsetHost[nID]];
      ...
      if (bOKToOutput) {
        nGrayLevel = 240;
        while (nContourPixelCount <
               static_cast<int>(nMaxContourPixelCount - 1)) {
          nCurPixelX = pCurContoursPixelGeometryListHost[nContourPixelCount]
                           .oContourOrderedGeometryLocation.x;
          nCurPixelY = pCurContoursPixelGeometryListHost[nContourPixelCount]
                           .oContourOrderedGeometryLocation.y;
          //                    if (nCurPixelX >= 0)
          printf("ID %d Cnt %d %d %d  \n", nID, nContourPixelCount, nCurPixelX,
                 nCurPixelY);
          if (nCurPixelX >= 0 && nCurPixelY >= 0) {
            pContoursOrderedGeometryImageHost[oSizeROI.width * nCurPixelY +
                                              nCurPixelX] = nGrayLevel;
            if (nGrayLevel > 0)
              nGrayLevel -= 1;
            else
              nGrayLevel = 240;
    ...
    nSize = 0;
    for (int j = 0; j < oSizeROI.height; j++) {
      nSize += fwrite(&pContoursGeometryImageHost[j * oSizeROI.width],
                      sizeof(Npp8u), oSizeROI.width, bmpFile);
    }
    fclose(bmpFile);
  }
