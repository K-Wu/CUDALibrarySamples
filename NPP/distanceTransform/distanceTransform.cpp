...
// This sample uses raw format input and output images so that no image
// decoder/encoder is necessary.
const std::string &InputFile0 = Path + std::string("Dolphin1_313x317_8u.raw");
const std::string &InputFile1 =
    Path + std::string("TestImage3_diamond_64x64_8u.raw");

const std::string &OutputVoronoiFile0 =
    Path + std::string("DistanceTransformVoronoi_Dolphin1_626x317_16s.raw");
const std::string &OutputVoronoiFile1 =
    Path + std::string("DistanceTransformVoronoi_TestImage3_128x64_16s.raw");

const std::string &OutputTrueTransformFile0 =
    Path + std::string("DistanceTransformTrue_Dolphin1_313x317_32f.raw");
const std::string &OutputTrueTransformFile1 =
    Path +
    std::string("DistanceTransformTrue_TestImage3_diamond_64x64_32f.raw");

const std::string &OutputTruncatedTransformFile0 =
    Path + std::string("DistanceTransformTruncated_Dolphin1_313x317_16u.raw");
const std::string &OutputTruncatedTransformFile1 =
    Path +
    std::string("DistanceTransformTruncated_TestImage3_diamond_64x64_16u.raw");
...
int loadRaw8BitImage(Npp8u *pImage, int nImage) {
  FILE *rawInputFile;
  size_t nSize;
  ...
  if (rawInputFile == NULL)
    return -1;
  nSize = fread(pImage, 1,
                oImageSizeROI[nImage].width * oImageSizeROI[nImage].height,
                rawInputFile);
...
// Since NPP is a very large library you should statically link to it whenever
// possible and only to the NPP libraries that contain functions that you use
// along with nppcore (nppc).  This will eliminate what can be significant
// library load times when using dynamic libraries.
...
int main(int argc, char **argv) {
  ...
  nppStreamCtx.hStream = 0; // The NULL stream by default, set this to whatever
                            // your created stream ID is if not the NULL stream.
  ...
  pInputImage0_8u_Host = reinterpret_cast<Npp8u *>(
      malloc(nImage0Width * sizeof(Npp8u) * nImage0Height));
  pInputImage1_8u_Host = reinterpret_cast<Npp8u *>(
      malloc(nImage1Width * sizeof(Npp8u) * nImage1Height));
  ...
  pOutputTransformImage0_32f_Host = reinterpret_cast<Npp32f *>(
      malloc(nImage0Width * sizeof(Npp32f) * nImage0Height));
  pOutputTransformImage1_32f_Host = reinterpret_cast<Npp32f *>(
      malloc(nImage1Width * sizeof(Npp32f) * nImage1Height));

  pOutputTruncatedImage0_16u_Host = reinterpret_cast<Npp16u *>(
      malloc(nImage0Width * sizeof(Npp16u) * nImage0Height));
  pOutputTruncatedImage1_16u_Host = reinterpret_cast<Npp16u *>(
      malloc(nImage1Width * sizeof(Npp16u) * nImage1Height));

  pOutputVoronoiDiagram0_16s_Host = reinterpret_cast<Npp16s *>(
      malloc(nImage0Width * 2 * sizeof(Npp16s) * nImage0Height));
  pOutputVoronoiDiagram1_16s_Host = reinterpret_cast<Npp16s *>(
      malloc(nImage1Width * 2 * sizeof(Npp16s) * nImage1Height));
  ...
  for (int i = 0; i < 2; i++) {
    nppStatus = nppiDistanceTransformPBAGetBufferSize(oImageSizeROI[i],
                                                      &nScratchBufferSize);
  ...
  cudaMalloc((void **)&pInputImage0_8u_Device,
             oImageSizeROI[0].width * sizeof(Npp8u) * oImageSizeROI[0].height);

  cudaMalloc((void **)&pInputImage1_8u_Device,
  ...
  cudaMalloc((void **)&pOutputTransformImage0_32f_Device,
             oImageSizeROI[0].width * sizeof(Npp32f) * oImageSizeROI[0].height);

  cudaMalloc((void **)&pOutputTransformImage1_32f_Device,
             oImageSizeROI[1].width * sizeof(Npp32f) * oImageSizeROI[1].height);

  cudaMalloc((void **)&pOutputTruncatedImage0_16u_Device,
             oImageSizeROI[0].width * sizeof(Npp16u) * oImageSizeROI[0].height);

  cudaMalloc((void **)&pOutputTruncatedImage1_16u_Device,
             oImageSizeROI[1].width * sizeof(Npp16u) * oImageSizeROI[1].height);

  cudaMalloc((void **)&pOutputVoronoiDiagram0_16s_Device,
             oImageSizeROI[0].width * 2 * sizeof(Npp16s) *
                 oImageSizeROI[0].height);

  cudaMalloc((void **)&pOutputVoronoiDiagram1_16s_Device,
             oImageSizeROI[1].width * 2 * sizeof(Npp16s) *
                 oImageSizeROI[1].height);
  ...
  if (cudaMemcpy2DAsync(
          pInputImage0_8u_Device, oImageSizeROI[0].width * sizeof(Npp8u),
  ...
  if (nppiDistanceTransformPBA_8u16u_C1R_Ctx(
          pInputImage0_8u_Device, oImageSizeROI[0].width * sizeof(Npp8u),
          nMinSiteValue, nMaxSiteValue, 0, 0, 0, 0, 0, 0,
          pOutputTruncatedImage0_16u_Device,
          oImageSizeROI[0].width * sizeof(Npp16u), oImageSizeROI[0],
          pScratchDeviceBuffer[0], nppStreamCtx) != NPP_SUCCESS) {
  ...
  if (cudaMemcpy2DAsync(
          pInputImage1_8u_Device, oImageSizeROI[1].width * sizeof(Npp8u),
  ...
  if (nppiDistanceTransformPBA_8u32f_C1R_Ctx(
          pInputImage1_8u_Device, oImageSizeROI[1].width * sizeof(Npp8u),
          nMinSiteValue, nMaxSiteValue, 0, 0, 0, 0,
          pOutputVoronoiDiagram1_16s_Device,
          oImageSizeROI[1].width * 2 * sizeof(Npp16s),
          pOutputTransformImage1_32f_Device,
          oImageSizeROI[1].width * sizeof(Npp32f), oImageSizeROI[1],
          pScratchDeviceBuffer[3], nppStreamCtx) != NPP_SUCCESS) {
    // Did Cuda fail to launch ?
    cudaError = cudaGetLastError();
    shutDown();
    return -1;
  ...
  if (nppiDistanceTransformPBA_8u16u_C1R_Ctx(
          pInputImage0_8u_Device, oImageSizeROI[0].width * sizeof(Npp8u),
          nMinSiteValue, nMaxSiteValue, pOutputVoronoiDiagram0_16s_Device,
          oImageSizeROI[0].width * 2 * sizeof(Npp16s), 0, 0, 0, 0, 0, 0,
          oImageSizeROI[0], pScratchDeviceBuffer[1],
          nppStreamCtx) != NPP_SUCCESS) {
    // Did Cuda fail to launch ?
    cudaError = cudaGetLastError();
    shutDown();
    return -1;
  ...
  if (nppiDistanceTransformPBA_8u16u_C1R_Ctx(
          pInputImage1_8u_Device, oImageSizeROI[1].width * sizeof(Npp8u),
          nMinSiteValue, nMaxSiteValue, 0, 0, 0, 0, 0, 0,
          pOutputTruncatedImage1_16u_Device,
          oImageSizeROI[1].width * sizeof(Npp16u), oImageSizeROI[1],
          pScratchDeviceBuffer[4], nppStreamCtx) != NPP_SUCCESS) {
    // Did Cuda fail to launch ?
    cudaError = cudaGetLastError();
    shutDown();
    return -1;
  ...
  if (nppiDistanceTransformPBA_8u32f_C1R_Ctx(
          pInputImage0_8u_Device, oImageSizeROI[0].width * sizeof(Npp8u),
          nMinSiteValue, nMaxSiteValue, 0, 0, 0, 0, 0, 0,
          pOutputTransformImage0_32f_Device,
          oImageSizeROI[0].width * sizeof(Npp32f), oImageSizeROI[0],
          pScratchDeviceBuffer[2], nppStreamCtx) != NPP_SUCCESS) {
    // Did Cuda fail to launch ?
    cudaError = cudaGetLastError();
    shutDown();
    return -1;
  ...
  // Normally you would continue on with whatever other operations you can do
  // while the images are in device memory. But in this case we are going to
  // copy all of the results back to host memory to save in output files. These
  // copy operations could be interleaved between the NPP calls but you would
  // want to verify that doing so does not decrease overall performance. These
  // copy operations could also be sent down another Cuda stream but then there
  // is no automatic synchronization so calls to cudaStreamSynchronize and
  // potentially Cuda events would be needed to guarantee proper
  // synchronization.

  // Copy back image0 true truncated transform result
  cudaError = cudaMemcpy2DAsync(
      pOutputTruncatedImage0_16u_Host, oImageSizeROI[0].width * sizeof(Npp16u),
      pOutputTruncatedImage0_16u_Device,
      oImageSizeROI[0].width * sizeof(Npp16u),
      oImageSizeROI[0].width * sizeof(Npp16u), oImageSizeROI[0].height,
      cudaMemcpyDeviceToHost, nppStreamCtx.hStream);

  // Copy back image1 voronoi diagram result
  cudaError = cudaMemcpy2DAsync(pOutputVoronoiDiagram1_16s_Host,
                                oImageSizeROI[1].width * 2 * sizeof(Npp16s),
                                pOutputVoronoiDiagram1_16s_Device,
                                oImageSizeROI[1].width * 2 * sizeof(Npp16s),
                                oImageSizeROI[1].width * 2 * sizeof(Npp16s),
                                oImageSizeROI[1].height, cudaMemcpyDeviceToHost,
                                nppStreamCtx.hStream);

  // Copy back image1 true transform result
  cudaError = cudaMemcpy2DAsync(
      pOutputTransformImage1_32f_Host, oImageSizeROI[1].width * sizeof(Npp32f),
      pOutputTransformImage1_32f_Device,
      oImageSizeROI[1].width * sizeof(Npp32f),
      oImageSizeROI[1].width * sizeof(Npp32f), oImageSizeROI[1].height,
      cudaMemcpyDeviceToHost, nppStreamCtx.hStream);

  // Copy back image0 voronoi diagram result
  cudaError = cudaMemcpy2DAsync(pOutputVoronoiDiagram0_16s_Host,
                                oImageSizeROI[0].width * 2 * sizeof(Npp16s),
                                pOutputVoronoiDiagram0_16s_Device,
                                oImageSizeROI[0].width * 2 * sizeof(Npp16s),
                                oImageSizeROI[0].width * 2 * sizeof(Npp16s),
                                oImageSizeROI[0].height, cudaMemcpyDeviceToHost,
                                nppStreamCtx.hStream);

  // Copy back image1 true transform result
  cudaError = cudaMemcpy2DAsync(
      pOutputTransformImage0_32f_Host, oImageSizeROI[0].width * sizeof(Npp32f),
      pOutputTransformImage0_32f_Device,
      oImageSizeROI[1].width * sizeof(Npp32f),
      oImageSizeROI[1].width * sizeof(Npp32f), oImageSizeROI[1].height,
      cudaMemcpyDeviceToHost, nppStreamCtx.hStream);
  ...
  cudaError = cudaMemcpy2DAsync(
      pOutputTruncatedImage1_16u_Host, oImageSizeROI[1].width * sizeof(Npp16u),
      pOutputTruncatedImage1_16u_Device,
      oImageSizeROI[1].width * sizeof(Npp16u),
      oImageSizeROI[1].width * sizeof(Npp16u), oImageSizeROI[1].height,
      cudaMemcpyDeviceToHost, nppStreamCtx.hStream);

  // Since we are saving the result images to files we need to add a
  // synchronization here to make sure that the copies have finished before
  ...
  for (int j = 0; j < oImageSizeROI[0].height; j++) {
    nSize +=
        fwrite(&pOutputTruncatedImage0_16u_Host[j * oImageSizeROI[0].width],
               sizeof(Npp16u), oImageSizeROI[0].width, rawOutputFile);
  }
  ...
  for (int j = 0; j < oImageSizeROI[1].height; j++) {
    nSize +=
        fwrite(&pOutputVoronoiDiagram1_16s_Host[j * oImageSizeROI[1].width * 2],
               2 * sizeof(Npp16s), 2 * oImageSizeROI[1].width, rawOutputFile);
  }
  fclose(rawOutputFile);
  ...
  for (int j = 0; j < oImageSizeROI[1].height; j++) {
    nSize +=
        fwrite(&pOutputTransformImage1_32f_Host[j * oImageSizeROI[1].width],
               sizeof(Npp32f), oImageSizeROI[1].width, rawOutputFile);
  }
  fclose(rawOutputFile);
  ...
  for (int j = 0; j < oImageSizeROI[0].height; j++) {
    nSize +=
        fwrite(&pOutputVoronoiDiagram0_16s_Host[j * oImageSizeROI[0].width * 2],
               sizeof(Npp16s), 2 * oImageSizeROI[0].width, rawOutputFile);
  }
  fclose(rawOutputFile);
  ...
  for (int j = 0; j < oImageSizeROI[1].height; j++) {
    nSize +=
        fwrite(&pOutputTruncatedImage1_16u_Host[j * oImageSizeROI[1].width],
               sizeof(Npp16u), oImageSizeROI[1].width, rawOutputFile);
  }
  fclose(rawOutputFile);
  ...
  for (int j = 0; j < oImageSizeROI[0].height; j++) {
    nSize +=
        fwrite(&pOutputTransformImage0_32f_Host[j * oImageSizeROI[0].width],
               sizeof(Npp32f), oImageSizeROI[0].width, rawOutputFile);
  }
  fclose(rawOutputFile);
