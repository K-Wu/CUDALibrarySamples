...
//        the relative brightness of each label value.
//
...
const std::string &InputFile1 =
    Path + std::string("CT_skull_512x512_8u_Gray.raw");
...
const std::string &InputFile4 =
    Path + std::string("coins_overlay_500x569_8u_Gray.raw");

const std::string &SegmentsOutputFile0 =
    Path + std::string("Lena_Segments_8Way_512x512_8u.raw");
const std::string &SegmentsOutputFile1 =
    Path + std::string("CT_skull_Segments_8Way_512x512_8u.raw");
const std::string &SegmentsOutputFile2 =
    Path + std::string("Rocks_Segments_8Way_512x512_8u.raw");
const std::string &SegmentsOutputFile3 =
    Path + std::string("coins_Segments_8Way_500x383_8u.raw");
const std::string &SegmentsOutputFile4 =
    Path + std::string("coins_overlay_segments_500x569_8u.raw");

const std::string &SegmentBoundariesOutputFile0 =
    Path + std::string("Lena_SegmentBoundaries_8Way_512x512_8u.raw");
const std::string &SegmentBoundariesOutputFile1 =
    Path + std::string("CT_skull_SegmentBoundaries_8Way_512x512_8u.raw");
const std::string &SegmentBoundariesOutputFile2 =
    Path + std::string("Rocks_SegmentBoundaries_8Way_512x512_8u.raw");
const std::string &SegmentBoundariesOutputFile3 =
    Path + std::string("coins_SegmentBoundaries_8Way_500x383_8u.raw");
const std::string &SegmentBoundariesOutputFile4 =
    Path + std::string("coins_overlay_SegmentBoundaries_8Way_500x569_8u.raw");

const std::string &SegmentsWithContrastingBoundariesOutputFile0 =
    Path +
    std::string("Lena_SegmentsWithContrastingBoundaries_8Way_512x512_8u.raw");
...
const std::string &SegmentsWithContrastingBoundariesOutputFile2 =
    Path +
    std::string("Rocks_SegmentsWithContrastingBoundaries_8Way_512x512_8u.raw");
const std::string &SegmentsWithContrastingBoundariesOutputFile3 =
    Path +
    std::string("coins_SegmentsWithContrastingBoundaries_8Way_500x383_8u.raw");
const std::string &SegmentsWithContrastingBoundariesOutputFile4 =
    Path +
    std::string(
        "coins_overlay_SegmentsWithContrastingBoundaries_8Way_500x569_8u.raw");

const std::string &CompressedSegmentLabelsOutputFile0 =
    Path + std::string("Lena_CompressedSegmentLabels_8Way_512x512_32u.raw");
const std::string &CompressedSegmentLabelsOutputFile1 =
    Path + std::string("CT_skull_CompressedSegmentLabels_8Way_512x512_32u.raw");
const std::string &CompressedSegmentLabelsOutputFile2 =
    Path + std::string("Rocks_CompressedSegmentLabels_8Way_512x512_32u.raw");
const std::string &CompressedSegmentLabelsOutputFile3 =
    Path + std::string("coins_CompressedSegmentLabels_8Way_500x383_32u.raw");
const std::string &CompressedSegmentLabelsOutputFile4 =
    Path +
    std::string("coins_overlay_CompressedSegmentLabels_8Way_500x569_32u.raw");
...
int main(int argc, const char *argv[]) {
  ...
  if ((pidx = findParamIndex(argv, argc, "-h")) != -1 ||
    ...
    std::cout << "Usage: " << argv[0] << "[-b number-of-batch]\n";
  ...
  for (int nImage = 0; nImage < params.numofbatch; nImage++) {
    ...
    // cudaMallocPitch OR cudaMalloc can be used here, in this sample case width
    // == pitch.
    ...
    // Output label marker buffers are only needed if you want to same the
    // generated segmentation labels, they ARE compatible with NPP UF generated
    // labels. Requesting segmentation output may slightly decrease segmentation
    // function performance.  Regardless of the pitch of the segmentation image
    // the segment labels output buffer will have a pitch of
    // oSizeROI[nImage].width * sizeof(Npp32u).

    aSegmentLabelsOutputBufferSize[nImage] =
        oSizeROI[nImage].width * sizeof(Npp32u) * oSizeROI[nImage].height;

    cudaError = cudaMalloc((void **)&pSegmentLabelsOutputBufferDev[nImage],
                           aSegmentLabelsOutputBufferSize[nImage]);
    if (cudaError != cudaSuccess)
      return NPP_MEMORY_ALLOCATION_ERR;

    pSegmentLabelsOutputBufferHost[nImage] = reinterpret_cast<Npp32u *>(malloc(
        oSizeROI[nImage].width * sizeof(Npp32u) * oSizeROI[nImage].height));
    ...
    if (loadRaw8BitImage(pInputImageHost[nImage],
      ...
      // Make a second copy of the unaltered input image since this function
      // works in place and we want to reuse the input image multiple times.
      cudaError = cudaMemcpy2DAsync(
          pSegmentsDev[nImage], oSizeROI[nImage].width * sizeof(Npp8u),
          pInputImageHost[nImage], oSizeROI[nImage].width * sizeof(Npp8u),
          oSizeROI[nImage].width * sizeof(Npp8u), oSizeROI[nImage].height,
          cudaMemcpyHostToDevice, nppStreamCtx.hStream);
      ...
      nppStatus = nppiSegmentWatershed_8u_C1IR_Ctx(
          ...
          oSizeROI[nImage].width * sizeof(Npp32u), eNorm,
          NPP_WATERSHED_SEGMENT_BOUNDARIES_NONE, oSizeROI[nImage],
          pSegmentationScratchBufferDev[nImage], nppStreamCtx);
      ...
      nppStatus = nppiCompressMarkerLabelsUF_32u_C1IR(
          pSegmentLabelsOutputBufferDev[nImage],
          oSizeROI[nImage].width * sizeof(Npp32u), oSizeROI[nImage],
          oSizeROI[nImage].width * oSizeROI[nImage].height,
          &nCompressedLabelCount, pCompressedLabelsScratchBufferDev);
      ...
      cudaError = cudaMemcpy2DAsync(
          pSegmentsHost[nImage], oSizeROI[nImage].width * sizeof(Npp8u),
          pSegmentsDev[nImage], oSizeROI[nImage].width * sizeof(Npp8u),
          oSizeROI[nImage].width * sizeof(Npp8u), oSizeROI[nImage].height,
      ...
      for (int j = 0; j < oSizeROI[nImage].height; j++) {
        nSize += fwrite(&pSegmentsHost[nImage][j * oSizeROI[nImage].width],
                        sizeof(Npp8u), oSizeROI[nImage].width, bmpFile);
      }
      fclose(bmpFile);
      ...
      for (int j = 0; j < oSizeROI[nImage].height; j++) {
        nSize += fwrite(
            &pSegmentLabelsOutputBufferHost[nImage][j * oSizeROI[nImage].width],
      ...
      if (nImage == 0)
        ...
        printf("coins_overlay_CompressedSegmentLabels_8Way_500x569_32u "
               "succeeded.\n");
      ...
      nppStatus = nppiSegmentWatershed_8u_C1IR_Ctx(
          pSegmentsDev[nImage], oSizeROI[nImage].width * sizeof(Npp8u), 0, 0,
          eNorm, NPP_WATERSHED_SEGMENT_BOUNDARIES_ONLY, oSizeROI[nImage],
          pSegmentationScratchBufferDev[nImage], nppStreamCtx);
      ...
      nppStatus = nppiSegmentWatershed_8u_C1IR_Ctx(
          pSegmentsDev[nImage], oSizeROI[nImage].width * sizeof(Npp8u), 0, 0,
          eNorm, NPP_WATERSHED_SEGMENT_BOUNDARIES_CONTRAST, oSizeROI[nImage],
          pSegmentationScratchBufferDev[nImage], nppStreamCtx);

      if (nppStatus != NPP_SUCCESS) {
        if (nImage == 0)
          printf("Lena segments with contrasting boundaries 8Way 512x512 8u "
                 "failed.\n");
        else if (nImage == 1)
          printf("CT skull segments with contrasting boundaries 8Way 512x512 "
                 "8u failed.\n");
        else if (nImage == 2)
          printf("Rocks segments with contrasting boundaries 8Way 512x512 8u "
                 "failed.\n");
        else if (nImage == 3)
          printf("coins segments with contrasting boundaries 8Way 500x383 8u "
                 "failed.\n");
        else if (nImage == 4)
          printf("coins overlay segments with contrasting boundaries 8Way "
                 "500x569 8u failed.\n");
        tearDown();
        return -1;
      }
      ...
      if (nImage == 0)
        fopen_s(&bmpFile, SegmentsWithContrastingBoundariesOutputFile0.c_str(),
                "wb");
      else if (nImage == 1)
        fopen_s(&bmpFile, SegmentsWithContrastingBoundariesOutputFile1.c_str(),
                "wb");
      else if (nImage == 2)
        fopen_s(&bmpFile, SegmentsWithContrastingBoundariesOutputFile2.c_str(),
                "wb");
      else if (nImage == 3)
        fopen_s(&bmpFile, SegmentsWithContrastingBoundariesOutputFile3.c_str(),
                "wb");
      else if (nImage == 4)
        fopen_s(&bmpFile, SegmentsWithContrastingBoundariesOutputFile4.c_str(),
                "wb");
      ...
      if (nImage == 0)
        printf("Lena_SegmentsWithContrastingBoundaries_8Way_512x512_8u "
               "succeeded.\n");
      else if (nImage == 1)
        printf("CT_Skull_SegmentsWithContrastingBoundaries_8Way_512x512_8u "
               "succeeded.\n");
      else if (nImage == 2)
        printf("Rocks_SegmentsWithContrastingBoundaries_8Way_512x512_8u "
               "succeeded.\n");
      else if (nImage == 3)
        printf("coins_SegmentsWithContrastingBoundaries_8Way_500x383_8u "
               "succeeded.\n");
      else if (nImage == 4)
        printf("coins_overlay_SegmentsWithContrastingBoundaries_8Way_500x569_"
               "8u succeeded.\n");
    }
  }
