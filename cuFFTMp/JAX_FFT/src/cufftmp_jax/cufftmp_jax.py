...
def _cufftmp_bind(input, num_parts, dist, dir):
    ...
    (output,) = _cufftmp_prim.bind(input, num_parts=num_parts, dist=dist, dir=dir)

    # scale in INVERSE direction
    if dir == Dir.INV:
        fft_dims = dist.fft_shape(input.shape)
        output = jax.numpy.divide(
            output, math.prod([jax.numpy.complex64(f) for f in fft_dims])
        )
...
def _partition(arg_shapes, arg_shardings, result_shape, result_sharding, dist, dir):
    ...
    return (
        lambda x: _cufftmp_bind(
            x, num_parts=jax.device_count(), dist=dist, dir=dir),
        _supported_sharding(arg_shardings[0], dist.opposite),
        [_supported_sharding(arg_shardings[0], dist)],
    )


def _infer_sharding_from_operands(arg_shapes, arg_shardings, result_shape, dist, dir):
...
def _cufftmp_translation(ctx, input, num_parts, dist, dir):
    ...
    output_type = mlir.ir.RankedTensorType.get(
        dims_out, input_type.element_type)

    layout = tuple(range(len(dims_in) - 1, -1, -1))

    if len(fft_dims) == 2:
        opaque = gpu_ops.build_cufftmp_descriptor(
            fft_dims[0], fft_dims[1], 1, dist._C_enum, dir._C_enum
        )
    elif len(fft_dims) == 3:
        opaque = gpu_ops.build_cufftmp_descriptor(
            fft_dims[0], fft_dims[1], fft_dims[2], dist._C_enum, dir._C_enum
        )
    else:
