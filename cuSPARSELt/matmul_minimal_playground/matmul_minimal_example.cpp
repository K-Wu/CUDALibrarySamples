...
int main(void) {
    ...
    CHECK_CUSPARSE( cusparseLtStructuredDescriptorInit(
                                            &handle, &(matA->mat), num_A_rows,
                                            num_A_cols, lda, alignment,
                                            type, order,
                                            CUSPARSELT_SPARSITY_50_PERCENT) )
    CHECK_CUSPARSE( cusparseLtDenseDescriptorInit(
                                            &handle, &(matB->mat), num_B_rows,
                                            num_B_cols, ldb, alignment,
                                            type, order) )
    CHECK_CUSPARSE( cusparseLtDenseDescriptorInit(
                                            &handle, &(matC->mat), num_C_rows,
                                            num_C_cols, ldc, alignment,
                                            type, order) )
    // matmul, algorithm selection, and plan initialization
    CHECK_CUSPARSE( cusparseLtMatmulDescriptorInit(
                                            &handle, &(matA->matmul), opA, opB,
                                            &(matA->mat), &(matB->mat), &(matC->mat), &(matC->mat),
                                            compute_type) )
    ...
    CHECK_CUSPARSE( cusparseLtSpMMAPrune(&handle, &(matA->matmul), dA, matA->values,
                                         CUSPARSELT_PRUNE_SPMMA_TILE, stream) )
    CHECK_CUSPARSE( cusparseLtSpMMAPruneCheck(&handle, &(matA->matmul), matA->values,
                                              d_valid, stream) )
    int is_valid;
    ...
    CHECK_CUSPARSE( cusparseLtMatmulPlanInit(&handle, &(matA->plan), &(matA->matmul), &(matA->alg_sel)))
    ...
    CHECK_CUDA( cudaMalloc((void**) &dA_compressedBuffer,
                           compressed_buffer_size) )
    CHECK_CUSPARSE( cusparseLtSpMMACompress(&handle, &(matA->plan), dA, dA_compressed,
                                            dA_compressedBuffer,stream) )
    
    // Perform the matrix multiplication
    CHECK_CUSPARSE( cusparseLtMatmul(&handle, &(matA->plan), &alpha, dA_compressed, (matB->values),
                                     &beta, (matC->values), dD, d_workspace, streams,
                                     num_streams) )
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
