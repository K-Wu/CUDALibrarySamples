...
int main(const int argc, const char *argv[]) {
    ...
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void **>(&d_A_array), sizeof(data_type ) * m * n * batch_count));
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void **>(&d_B_array), sizeof(data_type ) *m * k * batch_count));
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void **>(&d_C_array), sizeof(data_type ) *n*k* batch_count));

    for (int i = 0; i < batch_count; i++) {
        CUDA_CHECK(cudaMemcpyAsync(&d_A_array[m*n], A_array[i].data(), sizeof(data_type) * A_array[i].size(),
                                   cudaMemcpyHostToDevice, stream));
        CUDA_CHECK(cudaMemcpyAsync(&d_B_array[m*k], B_array[i].data(), sizeof(data_type) * B_array[i].size(),
                                   cudaMemcpyHostToDevice, stream));
    }
    ...
    CUBLAS_CHECK(cublasSgemmStridedBatched(cublasH, transa, transb, m, n, k, &alpha, d_A_array, lda, m*n,
                                    d_B_array, ldb, m*k, &beta, d_C_array, ldc, k*n, batch_count));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    cudaDeviceSynchronize();
