...
int main(const int argc, const char **argv) {
  ...
  cudaHostRegister(hA, A_size * sizeof(float), cudaHostRegisterDefault);
  cudaHostRegister(hB, B_size * sizeof(float), cudaHostRegisterDefault);
  ...
  CHECK_CUSPARSE(cusparseCreateDnMat(&matA, A_num_rows, A_num_cols, lda, hA,
  ...
  CHECK_CUSPARSE(cusparseCreateCsr(
      &matC, A_num_rows, B_num_cols, C_nnz,
      ...
      (void *)thrust::raw_pointer_cast(dC.values.data()), CUSPARSE_INDEX_32I,
