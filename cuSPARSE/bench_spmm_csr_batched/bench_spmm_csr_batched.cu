...
int main(const int argc, const char **argv) {
  ...
  float beta = 0.0f;
  //--------------------------------------------------------------------------
  // TODO: remove dA since it is not used
  ...
  for (int idx = 0; idx < num_batches; idx++) {
    CHECK_CUDA(cudaMemcpy(dA_csrOffsets + idx * (A_num_rows + 1),
                          hA.row_offsets.data(), (A_num_rows + 1) * sizeof(int),
                          cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(dA_columns + idx * A_nnz, hA.column_indices.data(),
                          A_nnz * sizeof(int), cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(dA_values + idx * A_nnz, hA.values.data(),
  ...
  printf("cusparseSpMMBatched+CSR elapsed time (ms): %f\n", elapsed_time);
  float throughput =
      2.0 * A_nnz * B_num_cols * num_batches / (elapsed_time / 1000.0) / 1e9;
  printf("cusparseSpMMBatched+CSR throughput (GFLOPS): %f\n", throughput);
  printf(
